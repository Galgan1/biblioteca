...
def _checar_card(card, onde, motivos):
    ...
    emph = card.get("emph")
    if emph is not None:
        # emph="" é substring de tudo mas não destaca nada → exige não-vazio
        if not isinstance(emph, str) or not emph.strip() or emph not in t:
            motivos.append(f"{onde}: 'emph' {emph!r} inválido (vazio ou não-substring exata de 't')")
