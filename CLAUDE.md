...
- **Resiliência em todo cliente de API paga:** `@retry(...)` (fora) + `@circuit_breaker(api=...)` (dentro) — NESSA ordem — + `record_cost(api=...)` no sucesso. Completo em `imagen/tts_gcloud/falgen/veo`; **`upload_youtube` NÃO usa `@retry` de propósito** (re-subir = upload duplicado; resiliência por chunk). Estado do breaker em `canal-state.json["api_health"]`.
- **Pré-flight de APIs = `secrets_doctor.py` (o doutor previne; o breaker só reage).** Responde "as APIs trabalham corretamente?" com EVIDÊNCIA: para cada chave em `.secrets/` faz uma chamada GRÁTIS de status (ElevenLabs `/v1/user`, fal `upload_file`, NVIDIA `/v1/models`, Google TTS `/voices`) e classifica o bloqueio. Rode antes de gastar e na auditoria das 02h. **`classify(code)` é a taxonomia canônica** (não caçar o problema errado): `401/403`=conta/billing/key (NÃO é código, não re-tenta) · `402`=crédito · `404`=endpoint/nome de modelo mudou (caso `FAL_WAN_MODEL`/`UPSCALE_MODEL`) · `408/429/5xx`=transitório (retry/breaker resolvem). Cuidado: **auth ✅ ≠ endpoint liberado** (NVIDIA: LLM autentica mas imagem `genai` é entitlement à parte).
