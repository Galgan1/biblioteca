...
def lowcut(x, fc=40):
    ...
    if fc <= 0:
        return x
    n = len(x); nf = _nfft(n); f = np.fft.rfftfreq(nf, 1/SR)
...
def make_ir(decay=1.1, predelay=0.012, er_n=12, seed=1, dark=0.55):
    ...
    ir = np.zeros(L); pd = int(SR * predelay)
    ...
    f = np.fft.rfftfreq(L, 1/SR); fc = 2000 + 9000 * (1 - dark)
...
def chorus(x, depth_ms=6.0, rate=0.6, mix=0.3, seed=2):
    ...
    lfo = (depth_ms/1000) * (0.5 + 0.5*np.sin(2*np.pi*rate*t + np.random.default_rng(seed).random()*6.28))
    d = (0.008 + lfo) * SR
...
def master(x, lowcut_fc=40, sat=1.6, air_db=3.0, air_fc=8000,
           rev=0.25, decay=1.1, dark=0.55, predelay=0.012, seed=1, peak=0.9):
