...
def _load_wav(p):
    with wave.open(str(p)) as w:
        return (
            np.frombuffer(w.readframes(w.getnframes()), dtype=np.int16).astype(np.float64) / 32768
        )
...
def main(roteiro_path):
    ...
    for i in range(n):
        ...
        if not mp3.exists():
            sys.exit(
                f'[!] {mp3} ausente — gere o vídeo de {slug} antes (e não construa outro no meio).'
            )
    ...
    if voz.exists():
        ...
        if abs(vlen - total) > 0.6:
            sys.exit(
                f'[!] _work nao bate com os stems ({total:.1f}s vs voz {vlen:.1f}s) — rebuild {slug}.'
            )
    ...
    if 'climax_cena' in ef:
        climax = max(
            0, min(n_trans - 1, int(ef['climax_cena']) - 2)
        )  # entrada da cena S = virada S-2 (0-base)
    else:
    ...
    print(
        f'   arco de comoção (Fibonacci), clímax na virada {climax}'
