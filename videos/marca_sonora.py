...
def _t(dur): return np.arange(int(SR * dur)) / SR
...
def _band(sig, lo, hi):
    ...
    n = len(sig); f = np.fft.rfftfreq(n, 1/SR)
    S = np.fft.rfft(sig); S[(f < lo) | (f > hi)] = 0
...
def _bumbo(dur=0.18, f_hi=120, f_lo=45, decay=0.085, click=0.22):
    ...
    n = int(SR*0.02); c = np.diff(rng.standard_normal(n), prepend=0)
    c *= np.exp(-np.arange(n)/(SR*0.0025)); c = c/(np.max(np.abs(c))+1e-9)*click
    body[:n] += c
    return body
...
def s01_assinatura():
    ...
    dur = 2.5; t = _t(dur); out = np.zeros(len(t))
    # leito grave (D3+A3+D4) que entra suave
    bed = (np.sin(2*np.pi*D3*t)+0.6*np.sin(2*np.pi*A3*t)+0.4*np.sin(2*np.pi*D4*t))
    bed *= np.clip(t/0.5, 0, 1) * np.exp(-np.maximum(0, t-1.4)/0.6)
    out += 0.35*bed
    def place(sig, at):
        s = int(at*SR); m = min(len(sig), len(out)-s); out[s:s+m] += sig[:m]
    ...
    place(0.12*_fade(_band(np.random.default_rng(1).standard_normal(int(SR*1.0)), 4000, 12000)*np.exp(-_t(1.0)/0.5)), 0.8)
    return _fade(out)
...
def s03_revelacao():
    ...
    dur = 1.2; t = _t(dur); out = np.zeros(len(t))
    ...
    out += 0.18*_band(np.random.default_rng(2).standard_normal(len(t)), 6000, 14000)*np.exp(-t/0.4)
    return _fade(out)

def s04_riser():
    """Riser de tensão (~2.0s): ruído filtrado subindo + tom subindo + crescendo."""
    dur = 2.0; t = _t(dur); out = np.zeros(len(t))
...
def s05_impacto():
    ...
    dur = 0.7; t = _t(dur); out = np.zeros(len(t))
    f = D2*(1+0.5*np.exp(-t/0.02))
    sub = np.sin(2*np.pi*np.cumsum(f)/SR) + 0.5*np.sin(2*np.pi*np.cumsum(f*0.5)/SR)
    braam = np.tanh(3*(np.sin(2*np.pi*D3*t)+np.sin(2*np.pi*A3*1.005*t)))  # grit harmônico
...
def s06_resolucao():
    ...
    dur = 1.8; t = _t(dur); out = np.zeros(len(t))
...
def s07_pagina():
    ...
    env = np.exp(-((t-0.10)/0.05)**2) + 0.7*np.exp(-((t-0.26)/0.06)**2)  # "shff-shff"
...
def s08_tick():
    ...
    click = np.diff(np.random.default_rng(9).standard_normal(len(t)), prepend=0)*np.exp(-t/0.0015)
...
def s09_roomtone():
    ...
    brown = _band(brown, 40, 350); brown /= (np.max(np.abs(brown))+1e-9)
...
def s10_encerramento():
    ...
    dur = 2.3; t = _t(dur); out = np.zeros(len(t))
    pad = (np.sin(2*np.pi*D3*t)+0.6*np.sin(2*np.pi*A3*t)+0.4*np.sin(2*np.pi*D4*t))
    ...
    place(0.10*_band(np.random.default_rng(11).standard_normal(int(SR*1.2)), 4000, 12000)*np.exp(-_t(1.2)/0.6), 0.6)
    return _fade(out)


SONS = [
    ('01_assinatura', s01_assinatura), ('02_transicao', s02_transicao),
    ('03_revelacao', s03_revelacao),   ('04_riser', s04_riser),
    ('05_impacto', s05_impacto),       ('06_resolucao', s06_resolucao),
    ('07_pagina', s07_pagina),         ('08_tick', s08_tick),
    ('09_roomtone', s09_roomtone),     ('10_encerramento', s10_encerramento),
]
...
def render_reel(rendered=None):
    ...
    reel = []
    for name, _ in SONS:
        reel.append(rendered[name]); reel.append(gap)
