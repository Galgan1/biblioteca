...
def _diagnostico(ctr, retencao):
    ...
    if ctr is not None:
        if ctr >= CTR_BOM:
            ...
            notas.append(
                f'CTR ⚠ {ctr:.1f}% — dentro da faixa, mas há espaço para melhorar o título'
            )
            lic_f1.append('Testar ângulo de tensão/paradoxo no título (CTR abaixo do teto)')
        else:
            alertas.append(
                f'CTR ✗ {ctr:.1f}% — abaixo de {CTR_OK}%: empacotamento fraco para o conteúdo'
            )
            lic_f1.append(
                'Priorizar ângulo de curiosidade ou benefício direto no próximo empacotamento'
            )
            lic_f1.append(
                'Revisar conceito de thumbnail — thumb e título devem gerar loop de curiosidade'
            )
    ...
    if retencao is not None:
        if retencao >= RET_BOA:
            notas.append(
                f'Retenção ✓ {retencao:.1f}% — arco dramático sustenta; padrão de clímax funcionou'
            )
        elif retencao >= RET_OK:
            notas.append(
                f'Retenção ⚠ {retencao:.1f}% — aceitável, mas revisar densidade nas cenas do meio'
            )
            lic_prod.append('Revisar loops de retenção a cada 15–30s nas cenas intermediárias')
        else:
            alertas.append(
                f'Retenção ✗ {retencao:.1f}% — abaixo de {RET_OK}%: estrutura precisa revisão'
            )
            lic_prod.append(
                'Reduzir duração das cenas conceituais; adicionar tensão/gap antes da resolução'
            )
...
def gerar(verbose=False):
    ...
    publicos = {
        vid: v for vid, v in yt.items() if v.get('privacyStatus') == 'public' and 'erro' not in v
    }
    ...
    for vid, v in sorted(
        publicos.items(), key=lambda x: int(x[1].get('viewCount') or 0), reverse=True
    ):
        ctr = v.get('ctr')
    ...
    proxima = {
        'aviso': (
            f'Canal com {len(publicos)} vídeo(s) — pouco dado para regras definitivas. '
            'Trate como hipóteses.'
        )
        if len(publicos) < 3
        else None,
        'empacotamento': list(
            dict.fromkeys([l for v in por_video.values() for l in v['licoes_fase1']])
        )[:4],
        'producao': list(
            dict.fromkeys([l for v in por_video.values() for l in v['licoes_producao']])
        )[:4],
        'o_que_funcionou': [
            v['notas'][0] for v in por_video.values() if v['notas'] and '✓' in v['notas'][0]
        ][:3],
    }
...
def _print_report(r):
    ...
    print(
        f"Views total: {c['total_views']:,} | CTR médio: {c['ctr_medio_canal']}% | Retenção média: {c['retencao_media_canal']}%"
    )
    print(f"{'=' * 60}")
