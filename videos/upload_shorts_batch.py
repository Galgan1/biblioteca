...
SHORTS = [
    ('arte-da-guerra_01.mp4', 'A maior vitória não é vencer a batalha #Shorts', ARTE,
     ['a arte da guerra', 'sun tzu', 'estratégia', 'shorts', 'livros']),
    ('arte-da-guerra_06.mp4', 'A energia vem da posição, não do esforço #Shorts', ARTE,
     ['a arte da guerra', 'sun tzu', 'estratégia', 'foco', 'shorts']),
    ('arte-da-guerra_09.mp4', 'O exército que se move como uma serpente #Shorts', ARTE,
     ['a arte da guerra', 'sun tzu', 'liderança', 'equipe', 'shorts']),
    ('arte-da-guerra_10.mp4', 'As 5 fraquezas que destroem um líder #Shorts', ARTE,
     ['a arte da guerra', 'sun tzu', 'liderança', 'autoconhecimento', 'shorts']),
    ('maquiavel-pedagogo_02.mp4', 'O experimento de 1 dólar que inverteu a psicologia #Shorts', MAQ,
     ['maquiavel pedagogo', 'pascal bernardin', 'psicologia', 'educação', 'shorts']),
    ('maquiavel-pedagogo_03.mp4', 'Por que você obedece sentindo que escolheu #Shorts', MAQ,
     ['maquiavel pedagogo', 'pascal bernardin', 'psicologia', 'educação', 'shorts']),
    ('maquiavel-pedagogo_06.mp4', 'A manobra em três tempos para controlar valores #Shorts', MAQ,
     ['maquiavel pedagogo', 'pascal bernardin', 'educação', 'sociedade', 'shorts']),
    ('maquiavel-pedagogo_10.mp4', 'E se a queda do ensino não for um fracasso? #Shorts', MAQ,
     ['maquiavel pedagogo', 'pascal bernardin', 'educação', 'sociedade', 'shorts']),
]
...
def main():
    ...
    for fname, title, parent, tags in SHORTS:
        path = SH / fname
        if not path.exists():
            print(f'[!] ausente: {fname}'); continue
        body = {
            'snippet': {'title': title[:100], 'description': desc(parent)[:5000],
