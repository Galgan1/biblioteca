...
for f in sorted(glob.glob(os.path.join(os.path.dirname(os.path.abspath(__file__)), "*_data.py"))):
    ...
    try:
        ...
        print("ERRO importando", mod, "->", e); continue
    for c in getattr(m, "CHAPTERS", []):
        rows.append((len(c.get("cards", [])), mod, c.get("slug", "?"),
                     1 if c.get("lessons") else 0))
...
print(f"total capítulos: {len(rows)} | magros(<=4): {sum(1 for n,_,_,_ in rows if n<=4)} | (<=3): {sum(1 for n,_,_,_ in rows if n<=3)}")
